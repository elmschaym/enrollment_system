...
<script>
	...
	export default {
		...
		methods: {
			...
			login() {
				...
				if (api_config.hasOwnProperty('server')) {
					...
					this.$http.post(api_config.server.slice(0,-4) +'/login/', qs.stringify({ uname, passw })).then( res => {
						if (res.data.hasOwnProperty('api_token') && res.data.system_app_role == 'DEP') {
			...
			fetchAcadPreference() {
				this.$http.get('academic_preference/').then( res => {
					this.$storageSet("acad_prefer", res.data);
				});
			},
			winClose() {
				this.$sleep(500).then(() => {
		...
		created() {
			...
			window.nwWin.setMinimumSize(1000, 660);
			window.nwWin.setPosition('center');
			window.nwWin.transparent = false;
			let token = this.$storageGet('api_token', 'local') || "", user = this.$storageGet('user_info', 'local') || {};
			if (token != "" && user.hasOwnProperty('system_app_role') && user.system_app_role == 'DEP') {
				window.nwWin.hide();
				this.$router.push({ name: 'dbd-index', query: { set_dimen: 1 } });
			} else {
