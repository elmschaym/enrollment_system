	...
	<div class="form-c">
		<div class="w">
			...
			<div class="q">
				...
				<div class="s">
					<button :class="['br-confirm', isFormOkay ? 'okay' : '']" @click="printCOR" :disabled="isSavingForm || !isFormOkay"><v-icon name="print"></v-icon> Print COR</button>
				</div>
			</div>
		</div>
		<div id="print-cor">
			<print-cor v-if="isPrintable" :enrollment="enrollment" :tally="{ totalSubjects, allowedUnits, enrolledUnits }"></print-cor>
		</div>
	</div>
</template>

<script>
    import PrintCOR from '@/components/PrintCOR.vue';
	...
	export default {
		...
		methods: {
			...
			fetchEnrollment() {
				this.$http.get('admission/'+ this.admission.id +'/?type=for-cor&enrollment_fields=id,acad_year,acad_status,semester,max_units,is_confirmed,confirmed_at,billing&admission_fields=id,date_admitted,course,academic_program,student,enrollment&acadprogram_fields=name&course_fields=id,name,name_alias,program_type&student_fields=id,school_id,firstname,lastname,middlename,civil_status,religion,gender&billing_fields=id,sect_enrol,misc_bills&sectenrol_fields=id,subject,section&miscbill_fields=id,misc&misc_fields=id,name,amount&subject_fields=id,code,name,units&section_fields=id,name,sched_days,sched_time,room&room_fields=name&admissionf_status=CUR&billingf_to_bill=ENR&billingf_is_paid=true&enrollmentf_acad_year='+ this.calendar.year +'&enrollmentf_semester='+ this.calendar.semester).then(res => {
            ...
            printCOR() {
                this.$htmlToPaper('print-cor');
            }
		},
		created() {
			this.$store.commit('setModuleName', 'Admission – COR');
		}
	}
</script>
...
<style scoped>
	...
	.p .b {}
	.p .b .x {}
	.p .b .x .h, .p .b .x .g .i { display: grid; grid-template-columns: 28px 70px auto 80px 180px 70px 50px }
