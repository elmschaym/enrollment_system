...
<script>
	...
	export default {
			...
			login() {
				...
				if (api_config.hasOwnProperty('server')) {
					...
					this.$http.post(api_config.server.slice(0,-4) +'/login/', qs.stringify({ uname, passw })).then( res => {
						if (res.data.hasOwnProperty('api_token') && res.data.system_app_role == 'CAS') {
							this.$storageSet("user_info", res.data);
							this.$storageSet("api_token", res.data.api_token);
							this.fetchAppPreference();
							this.setDimensions();
							this.$router.push({ name: 'cas-index', query: { set_dimen: 1 } });
						} else {
		...
		created() {
			...
			if (token != "" && user.hasOwnProperty('system_app_role') && user.system_app_role == 'CAS') {
				window.nwWin.hide();
				this.$router.push({ name: 'cas-index', query: { set_dimen: 1 } });
			} else {
