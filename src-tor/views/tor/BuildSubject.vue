<template>
	<div class="wrap-m">
		<div class="w-m">
			<div class="p">
				<find-e-student @setValue="setStudIDFromSearch" :query="'studentf_is_new=0'"></find-e-student>
			</div>
			<div class="q">
	...
	export default {
		...
		methods: {
			setStudIDFromSearch(v) {
				this.student = v;
				this.$router.push({ name: 'build-subject-q', query: { stud_id: v.id }});
			},
			setCourseFromList(v) {
				this.course = v;
				this.subject = {};
		...
		mounted() {
			...
			this.$store.commit('setModuleName', 'Grades – Build Subjects');
