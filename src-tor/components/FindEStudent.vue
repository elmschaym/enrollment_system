    ...
    export default {
        ...
        props: {
            ...
            fields: {
                type: String,
                default() {
                    return 'id,school_id,firstname,middlename,lastname';
                }
            }
        },
        data() {
        ...
        methods: {
            fetchList() {
                ...
                this.$http.get('student/?action=finder&student_fields='+ this.fields +'&query='+ this.queryString +'&type='+ this.queryType +'&'+ this.query).then(res => {
                    if (res.status == 200)
                        this.list = res.data;
                    else
                        this.list = [];
...
<style scoped>
    ._f { background-color: #fff; box-shadow: 0 1px 1px rgba(0,0,0,0.24); height: 100%; }
    ...
    .find button { height: 28px; display: block; color: #111; padding: 8px 8px 8px 2px; border: none; border: 1px solid #555753; background-color: #555753; font-size: 11px; outline: none; cursor: pointer; color: #fff; }
    .find button svg { width: 12px; height: 12px; float: right; }

    .rslt {  position: relative; height: calc(100% - 30px); }
    .head { font-size: 8px; text-transform: uppercase; padding: 4px 12px; background-color: #f8f8f5; border-radius: 5px 5px 0 0 }

    .list { max-height: calc(100% - 20px); overflow-y: scroll; position: relative; }
