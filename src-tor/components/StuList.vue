<template>
	<div class="wrap-l">
		...
		<div class="list-o">
			<div class="data">
				<div class="tbl">
					...
					<div class="tbd" v-else-if="students.length > 0">
						<div :class="['ttr', selectId == l.id ? 'active' : '']" :key="l.school_id +'_'+ l.id" v-for="l in students" @contextmenu="studentListCMenu($event, l)">
			...
			<div class="page">
				<div class="l">
					<button @click="fetchStudents()">RELOAD</button>
				...
				<div class="r">
					<span><v-icon class="a" name="square"></v-icon> Old Student</span>
					<span><v-icon class="i" name="square"></v-icon> New Student</span>
				</div>
			</div>
		</div>
	</div>
	...
	export default {
		...
		methods: {
			...
			fetchStudents() {
				...
				this.$http.get('student/?action='+ this.queryAction +'&start='+ this.listStart +'&limit='+ this.listLimit +'&type='+ this.queryType +'&query='+ this.queryString +'&student_fields=id,school_id,firstname,middlename,lastname,gender,home_address,campus_address,civil_status,birthdate&studentf_is_new=0').then( res => { 
			...
			studentListCMenu(e, l) {
				...
				let cmenu = new window.nw.Menu(),
					items = [
						{ label: 'Build Subject', click: () => this.$router.push({ name: 'build-subject-q', query: { stud_id: l.id } }) },
...
<style scoped>
	...
	.list-o .data .tbl .ttd { padding: 10px 10px 10px 10px; height: 30px; font-size: 12px; text-overflow: clip; overflow: hidden; white-space: nowrap; }
	...
	.find-o .v input { width: 100%; height: 24px; color: #111; padding: 7px 10px; background-color: #fdfdfd; font-size: 11px; outline: none; border-right: none; border: 1px solid #959793; }
	.find-o .v svg { position: absolute; top: 6px; right: 8px; height: 12px; width: 12px; }
	.find-o .w {}
	.find-o .w button { width: 100%; height: 24px; color: #fff; padding: 5px 10px; background-color: #959793; font-size: 11px; border: 1px solid #959793; outline: none;  }
</style>
